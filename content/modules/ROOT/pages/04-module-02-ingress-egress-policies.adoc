...
ACME's development team has reported that their application routes stopped working after the `deny-all-incoming` policy was applied in Module 1. This is expected: the `deny-all-incoming` policy blocks all ingress traffic, including traffic from the OpenShift ingress controller that serves routes.
...
. Try to access the route using curl. This will fail because the `deny-all-incoming` policy blocks ingress controller traffic:
+
[source,bash,role=execute]
----
...
----
+
The command should time out or return an error. The `deny-all-incoming` policy is blocking the OpenShift ingress controller from reaching the hello pod.
...
* Blocking DNS will prevent pods from resolving service names
...
[source,bash,role=execute]
----
oc exec deploy/test -- curl -s --connect-timeout 5 -o /dev/null -w "%{http_code}\n" https://www.redhat.com/en
----
+
Expected output:
+
...
A deny-all egress policy that blocks DNS makes pods unable to resolve any service names. You need to allow DNS traffic and then selectively allow the destinations the test pod needs. OpenShift labels the OpenShift DNS namespace with kubernetes.io/metadata.name: openshift-dns. You can use this label in a namespaceSelector to allow traffic to the OpenShift DNS.
...
oc exec deploy/test -- curl -s --connect-timeout 5 -o /dev/null -w "%{http_code}\n" https://www.redhat.com || echo "Blocked as expected"
...
Module 3 will cover cross-namespace policies for managing network policies across a multi-team OpenShift environment.
