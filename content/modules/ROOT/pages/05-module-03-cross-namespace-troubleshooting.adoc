...
[source,bash,role=execute]
----
oc run reporting --image quay.io/redhattraining/hello-world-nginx:v1.0 -n frontend-team-%OPENSHIFT_USERNAME%
----
...
[source,bash,role=execute]
----
oc wait pod/reporting -n frontend-team-%OPENSHIFT_USERNAME% --for=condition=Ready --timeout=60s
----
...
----
BACKEND_SVC_IP=$(oc get svc backend-api -n backend-team-%OPENSHIFT_USERNAME% -o jsonpath='{.spec.clusterIP}')
oc exec reporting -n frontend-team-%OPENSHIFT_USERNAME% -- curl -s --connect-timeout 5 $BACKEND_SVC_IP:8080
----
+
...
[source,bash,role=execute]
----
oc get pod reporting -n frontend-team-%OPENSHIFT_USERNAME% --show-labels
----
+
...
[source,bash,role=execute]
----
oc get pods -n frontend-team-%OPENSHIFT_USERNAME% --show-labels
----
+
...
. Check the ingress policies more carefully. The `allow-from-frontend` policy uses a `namespaceSelector` without a `podSelector` under the `from` block, which means all pods in the frontend-team-{OPENSHIFT_USERNAME} namespace should be allowed.

=== Step 3: Check for egress restrictions

. Check if there are any network policies in the frontend-team-{OPENSHIFT_USERNAME} namespace:
+
[source,bash,role=execute]
----
oc get networkpolicies -n frontend-team-%OPENSHIFT_USERNAME%
----
+
...
[source,bash,role=execute]
----
BACKEND_POD_IP=$(oc get pod -l deployment=backend-api -n backend-team-%OPENSHIFT_USERNAME% -o jsonpath='{.items[0].status.podIP}')
echo "Backend pod IP: $BACKEND_POD_IP"
oc exec reporting -n frontend-team-%OPENSHIFT_USERNAME% -- curl -s --connect-timeout 5 $BACKEND_POD_IP:8080 | grep Hello
----
...
----
BACKEND_SVC_IP=$(oc get svc backend-api -n backend-team-%OPENSHIFT_USERNAME% -o jsonpath='{.spec.clusterIP}')
oc exec reporting -n frontend-team-%OPENSHIFT_USERNAME% -- curl -s --connect-timeout 5 $BACKEND_SVC_IP:8080 | grep Hello
----
...
[source,bash,role=execute]
----
oc get networkpolicies -n backend-team-%OPENSHIFT_USERNAME% -o wide
----
...
[source,bash,role=execute]
----
oc describe networkpolicies -n backend-team-%OPENSHIFT_USERNAME%
----
...
[source,bash,role=execute]
----
oc get networkpolicy allow-from-frontend -n backend-team-%OPENSHIFT_USERNAME% -o yaml
----
...
echo "=== test (network-policy-test-%OPENSHIFT_USERNAME%) -> backend-api (should fail) ==="
oc exec deploy/test -n network-policy-test-%OPENSHIFT_USERNAME% -- curl -s --connect-timeout 5 $BACKEND_SVC_IP:8080 || echo "Blocked as expected"

echo "=== test -> hello (network-policy-test-%OPENSHIFT_USERNAME%, should succeed) ==="
oc exec deploy/test -n network-policy-test-%OPENSHIFT_USERNAME% -- curl -s --connect-timeout 5 hello.network-policy-test-%OPENSHIFT_USERNAME%:8080 | grep Hello
----

Verification checklist:

* All pods in the frontend-team-{OPENSHIFT_USERNAME} namespace can reach backend-api
* Pods in other namespaces remain blocked from backend-api
* Module 1 and Module 2 policies in network-policy-test-{OPENSHIFT_USERNAME} still function correctly
